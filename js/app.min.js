...
self.libraries.push(library);},delay);});setTimeout(function(){self.inSearch(false);},totalDelay);});},function(error){var errorString;if(error.status===0){errorString="the request timed out please try again.";}else{errorString=error.statusText;}
console.warn('Foursquare: '+errorString);alert(errorString);});};self.showLibrary=function(library){if($(window).width()<768){toggleTable();}
animateAllMarkers(null);infowindow.close();library.mapMarker().setAnimation(google.maps.Animation.BOUNCE);infowindow=createInfowindowForLibrary(library);google.maps.event.addListener(infowindow,"closeclick",function(){animateAllMarkers(null);if($(window).width()<768){resizeMapToBounds(self.currentSuggestedBounds(),function(){});}});var latLng=library.mapMarker().getPosition();map.setCenter(latLng);google.maps.event.addListenerOnce(map,'idle',function(){infowindow.open(map,library.mapMarker());});};};var appVM=new AppViewModel();ko.applyBindings(appVM);