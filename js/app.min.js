...
return true;};var AppViewModel=function(){'use strict';var self=this;self.libraries=ko.observableArray();self.filteredLibraries=ko.observableArray();self.currentLocation=ko.observable();self.currentSuggestedBounds=ko.observable();self.currentTab=ko.observable('all');self.inSearch=ko.observable(false);self.query=ko.observable('');self.mapError=ko.observable(false);self.searchAll=function(){self.currentTab('all');self.searchLibraries();};self.searchOpen=function(){self.currentTab('open');self.searchLibraries();};self.searchTop=function(){self.currentTab('top');self.searchLibraries();};self.search=function(value){self.filteredLibraries.removeAll();for(var i in self.libraries()){self.libraries()[i]().mapMarker().setMap(map);if(self.libraries()[i]().name().toLowerCase().indexOf(value.toLowerCase())>=0){self.filteredLibraries.push(self.libraries()[i]);}else{self.libraries()[i]().mapMarker().setMap(null);}}};setCurrentLocation(function(pos){var crd=pos.coords;self.currentLocation({lat:crd.latitude,lng:crd.longitude});setMap(self.currentLocation(),function(){autocomplete.addListener('place_changed',function(){var place=autocomplete.getPlace();if(isPlaceValid(place)){var loc={lat:place.geometry.location.lat(),lng:place.geometry.location.lng(),};self.currentLocation(loc);self.searchLibraries();}else{alert('No Place found!, Please try another search term, or enter more specific address');}});self.searchLibraries();});},function(err){console.warn('ERROR('+err.code+'): '+err.message);self.currentLocation({lat:41.0417,lng:29.0094});setMap(self.currentLocation(),function(){autocomplete.addListener('place_changed',function(){var place=autocomplete.getPlace();if(isPlaceValid(place)){var loc={lat:place.geometry.location.lat(),lng:place.geometry.location.lng(),};self.currentLocation(loc);self.searchLibraries();}else{alert('No Place found!, Please try another search term, or enter more specific address');}});self.searchLibraries();});});self.searchLibraries=function(){self.inSearch(true);$.each(self.libraries(),function(index,library){library().mapMarker().setMap(null);});self.libraries.removeAll();self.filteredLibraries.removeAll();searchForLibrariesNearLocation(self.currentLocation(),self.currentTab(),function(data){var items=data.response.groups[0].items;if(items.length<1){self.inSearch(false);if(self.currentTab()==='open'){alert("No open libraries now!, please try again later.");}else{alert("No Libraries found!");}
return;}
var suggestedBounds=data.response.suggestedBounds;self.currentSuggestedBounds(suggestedBounds);map.setZoom(map.getZoom()+1);console.log('map zoom changed');resizeMapToBounds(self.currentSuggestedBounds(),function(){var totalDelay=items.length*50;$.each(items,function(index,item){var library=createLibraryFromItem(item);var delay=index*50;setTimeout(function(){if(self.currentTab()==='top'){creatMarker(library(),self.currentTab(),index+1);}else{creatMarker(library(),self.currentTab(),null);}
