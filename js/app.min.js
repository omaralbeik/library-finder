...
return true;};var AppViewModel=function(){var self=this;self.libraries=ko.observableArray();self.filteredLibraries=ko.observableArray();self.currentLocation=ko.observable();self.currentSuggestedBounds=ko.observable();self.latestLatLngBounds=ko.observable();self.currentTab=ko.observable('all');self.inSearch=ko.observable(false);self.query=ko.observable('');self.searchAll=function(){self.currentTab('all');self.searchLibraries();};self.searchOpen=function(){self.currentTab('open');self.searchLibraries();};self.searchTop=function(){self.currentTab('top');self.searchLibraries();};self.search=function(value){self.filteredLibraries.removeAll();for(var i in self.libraries()){if(self.libraries()[i]().name().toLowerCase().indexOf(value.toLowerCase())>=0){self.filteredLibraries.push(self.libraries()[i]);}}};setCurrentLocation(function(pos){var crd=pos.coords;self.currentLocation({lat:crd.latitude,lng:crd.longitude});setMap(self.currentLocation(),function(){autocomplete.addListener('place_changed',function(){var place=autocomplete.getPlace();if(isPlaceValid(place)){var loc={lat:place.geometry.location.lat(),lng:place.geometry.location.lng(),};self.currentLocation(loc);self.searchLibraries();}else{showNoPlaceFoundPopover();}});self.searchLibraries();});},function(err){console.warn('ERROR('+err.code+'): '+err.message);self.currentLocation({lat:41.0417,lng:29.0094});setMap(self.currentLocation(),function(){autocomplete.addListener('place_changed',function(){var place=autocomplete.getPlace();if(isPlaceValid(place)){var loc={lat:place.geometry.location.lat(),lng:place.geometry.location.lng(),};self.currentLocation(loc);self.searchLibraries();}else{showNoPlaceFoundPopover();}});self.searchLibraries();});});self.searchLibraries=function(){self.inSearch(true);$.each(self.libraries(),function(index,library){library().mapMarker().setMap(null);});self.libraries.removeAll();searchForLibrariesNearLocation(self.currentLocation(),self.currentTab(),function(data){var items=data.response.groups[0].items;if(items.length<1){self.inSearch(false);if(self.currentTab()==='open'){alert("No open libraries now!, please try again later.");}else{alert("No Libraries found!");}
return;}
...
self.libraries.push(library);self.filteredLibraries.push(library);},delay);});setTimeout(function(){self.inSearch(false);},totalDelay);});},function(error){var errorString;if(error.status===0){errorString="the request timed out please try again.";}else{errorString=error.statusText;}
